...
// ------------------------------------------------------------
function print_entire_month($year, $month) {
  global $size;
  global $monthly_day;
  global $monthly_hour;

  debug("print_entire_month($year, $month)");
...
function print_single_image($image_filename) {
  ...
  if (strlen($image_filename) < strlen("YYYYMMDDHHMMSSSS")) {
    // We do not have the hour or the minutes. This is the first image in a day (arrow-down from entire day).
...
function print_yesterday_tomorrow_links($timestamp, $is_entire_month) {
  ...
  if ($is_entire_month) {
    ...
    print "Forrige: <a href=\"?type=day&date=" . date('Ymd', $yesterday_timestamp) . "&size=$size\">" . date('Y-m-d', $yesterday_timestamp) . "</a>.\n";
    // Is there a tomorrow?
    ...
    // Link to the whole month at 13:00
    //------------------------------------------------------------
...
// ------------------------------------------------------------
function print_entire_day_link($timestamp) {
  $year= date('Y', $timestamp);
  $month = date('m', $timestamp);
  $day = date('d', $timestamp);
...
// ------------------------------------------------------------
function print_entire_day($timestamp, $image_size, $number_of_images) {
  global $max_images_row;
  global $size;
  
  // Get all *jpg images in "today's" image directory.
  $directory = date('Ymd', $timestamp);
  debug("print_entire_day($timestamp, $image_size, $number_of_images)");
...
$monthly_hour = 11; // 13:00 every day
...
if ($type == "last") {
  ...
  print_entire_day($timestamp, $size, $max_images);
} else if ($type == "month") {
  // All images for this month, spåecified with $year and $month parameters.
  if ($month && $year) {
    print_entire_month($year, sprintf("%02d", $month), $monthly_hour, $size); 
  }
} else {
  // Unknown type.
